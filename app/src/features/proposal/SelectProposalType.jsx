...
import { Settings, Warehouse, Package, ArrowRight } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Opção: Fornecimento de Material */}
        <button
          onClick={() => navigate('/propostas/nova/material')}
          className="bg-surface border-2 border-border p-8 rounded-3xl text-left hover:border-success hover:bg-success/5 transition-all group relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-32 h-32 bg-success/10 rounded-bl-full -z-10 group-hover:bg-success/20 transition-colors" />

          <div className="w-16 h-16 bg-bg border-2 border-border rounded-2xl flex items-center justify-center mb-6 group-hover:border-success transition-colors">
            <Package className="text-success" size={32} />
          </div>

          <h2 className="text-2xl font-bold font-syne text-white mb-3">Fornecimento de Material</h2>
          <p className="text-muted mb-6 leading-relaxed text-sm">
            Proposta dedicada ao fornecimento de materiais (PEAD, geomembrana, tubos, conexões) com seções de especificações técnicas, prazo de entrega e garantia.
          </p>

          <div className="flex items-center gap-2 font-bold text-success text-sm uppercase tracking-widest group-hover:translate-x-2 transition-transform">
            Iniciar Material <ArrowRight size={16} />
          </div>
        </button>
        ...
        >
          ...
          <p className="text-muted mb-6 leading-relaxed text-sm">
            Módulo específico para projetos estruturais de armazéns, com cálculo automatizado de obras e dimensionamento de materiais.
          </p>
